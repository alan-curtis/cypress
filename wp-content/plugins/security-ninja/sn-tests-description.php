...
#END WP Security Ninja - Prevent code in unexpected files</pre>
		...
		<div class="sn_test_details" id="feature_policy">
			<div class="test_name"><?php esc_html_e( 'Check if server response headers contain Feature-Policy', 'security-ninja' ); ?></div>
			<div class="test_description">
				...
				<p><?php esc_html_e( 'For a full and updated list check out the link.', 'security-ninja' ); ?> <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Feature-Policy" target="_blank" rel="noopener">Mozilla.org - Feature Policy</a></p>

				<p><?php esc_html_e( "Fixing is very easy. Open your theme's functions.php file and add the following:", 'security-ninja' ); ?></p>
<pre>header("Feature-Policy: accelerometer 'none' ; ambient-light-sensor 'none' ; autoplay 'none' ; camera 'none' ; encrypted-media 'none' ; fullscreen 'none' ; geolocation 'none' ; gyroscope 'none' ; magnetometer 'none' ; microphone 'none' ; midi 'none' ; payment 'none' ; speaker 'none' ; sync-xhr 'none' ; usb 'none' ; notifications 'none' ; vibrate 'none' ; push 'none' ; vr 'none' ");</pre>
...
<pre>
	#BEGIN WP Security Ninja - Set Feature-Policy
	&lt;IfModule mod_headers.c&gt;
	...
	&lt;/IfModule&gt;
	#END WP Security Ninja - Set Feature-Policy
</pre>
				<p><?php esc_html_e( 'For Nginx add this to the nginx.conf under server block', 'security-ninja' ); ?></p>
<pre>add_header Feature-Policy "accelerometer 'none'; autoplay 'none'; camera 'none'; encrypted-media 'none'; fullscreen 'none'; geolocation 'none'; gyroscope 'none'; magnetometer 'none'; microphone 'none'; midi 'none'; payment 'none'; sync-xhr 'none'; usb 'none';";</pre>
...
remove_action('template_redirect', 'rest_output_link_header', 11, 0);</pre>
		...
		<div class="sn_test_details" id="deactivated_themes">
			<div class="test_name">Check if there are any deactivated themes</div>
			<div class="test_description"><p>If you're not using a theme remove it from the WP themes folder. It's that simple. There's no reason to keep it there and in case the code is malicious or it has some vulnerabilities it can still be exploited by a hacker regardless of the fact the theme is not active.</p>
				<p>Open <a target="_blank" href="themes.php">Appearance - Themes</a> and simply delete all themes that are not active. Or login via FTP and move them to some folder that's not /wp-content/themes/.</p>
			</div>
		</div>
...
<div class="sn_test_details" id="upgrade_file_check">
	<div class="test_name"><?php esc_html_e( 'Check if the upgrade.php file is accessible via HTTP at the default location', 'security-ninja' ); ?></div>
	<div class="test_description"><p><?php esc_html_e( "There have already been a couple of security issues regarding this file. Besides the security issue it's never a good idea to let people run any database upgrade scripts without your knowledge. This is a useful file but it should not be accessible on the default location.", 'security-ninja' ); ?></p>
		<p><?php esc_html_e( 'This is a very easy problem to solve. Simply delete the file. You can also rename the file, move it or block access via .htaccess. We recommend the most foolproof and efficent method - delete the file.', 'security-ninja' ); ?></p>
	</div>
</div>
